...
void MATLABCaller::callHandlerAsync(const std::string& handlerName,
    ...
    std::function<void(ResponseData)> onComplete) {
...
void BlinkApp::handleRequest(ResType* res, ReqType* req, const Route& route) {
    ...
    res->onData([this, res, req, route, isAborted, bodyBuffer](std::string_view chunk, bool isLast) mutable {
        ...
        if (isLast) {
            processRequest(res, req, route, bodyBuffer->c_str(), isAborted);
        }
    });
...
void BlinkApp::processRequest(ResType* res,
    ...
    try {
        ...
        if (asyncMatlabHandlers_) {
            uWS::Loop* loop = webServer->getLoop();
            std::function<void(std::function<void()>)> deferToLoop =
                [loop](std::function<void()> fn) { loop->defer(std::move(fn)); };
            matlabCaller->callHandlerAsync(route.handlerName, request, deferToLoop,
                [this, res, cancelled](ResponseData response) {
